...
def Difference(image1, image2, debug=False):
    if debug:
    ...
    for col in range(len(image1)):
        for row in range(len(image1[col])):
            for color in range(len(image1[col][row])):
                if image1[col][row][color] < image2[col][row][color]:
                    output_image[col][row][color] = 1 + (image1[col][row][color] -
                                                         image2[col][row][color])
                else:
                    output_image[col][row][color] = 1 + (image2[col][row][color] -
